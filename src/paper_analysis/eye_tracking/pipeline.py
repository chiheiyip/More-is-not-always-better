...
def run_eye_pipeline(
    ...
    for _, row in scene.iterrows():
        ...
        if aoi_json and aoi_json.exists():
            ...
            _, metrics = compute_aoi_metrics(df, aois)
            validation = aoi_validation(aois, metrics)
        else:
            metrics = compute_whole_scene_metrics(df)
        ...
        qc_rows.append({**base, "missing_eye_file": False, "missing_aoi_file": not aoi_json or not aoi_json.exists(), "eye_sample_count": len(df)})
